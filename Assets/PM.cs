...
public class PM : MonoBehaviour
{
    ...
    private Vector3[] initialPositions; // 存储每个粒子的初始位置
    ...
    private void Start()
    {
        ...
        initialPositions = new Vector3[ps.main.maxParticles]; // 初始化初始位置数组
    ...
    private void Update()
    {
        ...
        {
            if (Time.time - lastChangeTime[i] >= 8f) // 检查是否已经过去了8秒
            {
                particles[i].position = new Vector3(initialPositions[i].x, particles[i].position.y, particles[i].position.z); // 重置x坐标
                lastChangeTime[i] = Time.time; // 重置上次改变时间
            }
